...
const App = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Firestoreへの登録に失敗しました：" + error.message);
    }
  };
    ...
    <div className="p-6 font-sans text-lg max-w-xl mx-auto">
      ...
      {view === "form" && (
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
          <input name="name" placeholder="名前" value={formData.name} onChange={handleChange} required className="p-2 border rounded" />
          <select name="department" value={formData.department} onChange={handleChange} className="p-2 border rounded">
          ...
          <input name="purpose" placeholder="使用目的" value={formData.purpose} onChange={handleChange} required className="p-2 border rounded" />
          <input name="guest" placeholder="来客者名" value={formData.guest} onChange={handleChange} className="p-2 border rounded" />
          <select name="room" value={formData.room} onChange={handleChange} className="p-2 border rounded">
          ...
          <input name="date" type="date" value={formData.date} onChange={handleChange} required className="p-2 border rounded" />
          <label>開始時間</label>
          <select name="startTime" value={formData.startTime} onChange={handleChange} required className="p-2 border rounded">
            {timeOptions.map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
          <label>終了時間</label>
          <select name="endTime" value={formData.endTime} onChange={handleChange} required className="p-2 border rounded">
            {timeOptions.map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
